...
const SizeTag = ({ size }: { size: string }) => {
  if (size.toLowerCase().trim() === "md") {
    return (
      <span className="px-[10px] py-[1px] rounded-lg bg-green-500/30 text-green-500 text-[14px]">
        {" "}
        {size}
      </span>
    );
    ...
    return (
      <span className="px-[10px] py-[1px] rounded-lg bg-yellow-500/30 text-yellow-500 text-[14px]">
        {" "}
        {size}
      </span>
    );
  ...
  if (size.toLowerCase().trim() === "md") {
    return (
      <span className="px-[10px] py-[1px] rounded-lg bg-pink-400/30 text-pink-400 text-[14px]">
        {" "}
        {size}
      </span>
    );
...
const InventoryProductDetails = ({ productId }: { productId: string }) => {
  const { product, sold, remaining, salesData } =
    useInventoryProductDetails(productId);
  if (!product) {
    return (
      <section className="w-full h-[100vh] overflow-y-auto gray-scroller">
        <h1 className="text-2xl font-bold my-[20px] p-5">Loading</h1>
      </section>
    );
  }
  return (
    <section className="w-full h-[100vh] overflow-y-auto gray-scroller">
      <h1 className="text-2xl font-bold my-[20px] p-5">
        {product.productName}
      </h1>
      ...
      <div className="p-5 flex flex-col gap-[10px]">
        ...
        <p>
          <span className="text-blue-600">{remaining}</span> pieces in stock
        </p>
        <p>
          <span className="text-blue-600">{sold}</span> pieces sold
        </p>
        <div className="flex flex-row flex-nowrap items-center gap-[20px]">
          <p>
            Ratings <span className="text-gray-400 text-sm">(5.0)</span>
          </p>
          <div className="flex flex-row flex-nowrap gap-[5px]">
            <Image
              src={"/images/star-ratings.svg"}
              alt="ratings"
              width={15}
              height={15}
            />
        ...
        <p>
          <span className="text-blue-600">{product.stockKeepingUnit}</span>{" "}
        ...
        <h1 className="text-lg font-bold my-[20px]">Update stocks :</h1>
        <div className="flex flex-row flex-nowrap gap-[10px]">
          <button
            type="button"
            className="px-[20px] py-[5px] rounded-md bg-blue-600"
          >
            + Increase quantity
          </button>
          <button
            type="button"
            className={`px-[20px] py-[5px] rounded-md bg-blue-600 ${product.availableSizes.includes("sm") && product.availableSizes.includes("md") && product.availableSizes.includes("lg") ? "" : "hidden"}`}
          >
            + Add sizes
          </button>
        </div>
