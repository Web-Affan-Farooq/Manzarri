...
const Stock = () => {
  ...
  useEffect(() => {
    const filtered = inventory.filter((product) =>
      product.jewelleryType.trim().toLowerCase().startsWith(category)
    );
      ...
      <div>
        ...
        <>
          <div className="flex flex-row flex-wrap gap-[20px] px-5 py-2">
            {data.map((productCategory, idx) => (
              <div
                className={`${category.toLowerCase() === productCategory.category ? "bg-amber-600/30 text-amber-600" : "bg-blue-600/30 text-blue-600"} cursor-pointer relative font-bold text-[14px] px-[15px] py-[2px] rounded-lg`}
                onClick={() => {
        ...
        <div className="flex flex-col">
          {StockProducts.length <= 0 ? (
            ...
            StockProducts.map((product: Product, idx: number) => (
