...
const Card = ({
  ...
  const { handleDelete, handleAccountBlockandUnblock, handleDeleteInput } =
    useAccountCard({
      userId: userId,
      isBlocked: isBlocked,
    });
  ...
  return (
    <div className="relative">
      {isAdmin ? (
        <span
          className={`absolute max-sm:right-14 sm:right-16 top-4 px-[9px] py-[2px] rounded-full text-[9px] tracking-[0.5px] font-semibold text-green-500 bg-green-500/40`}
        >
          Admin
        </span>
      ) : (
      ...
      <Dialog>
        <ContextMenu>
          ...
          <ContextMenuContent>
            <ContextMenuItem className="hover:bg-gray-400 cursor-pointer">
              Email
            </ContextMenuItem>
            <ContextMenuItem
              className={`${isAdmin ? "hidden" : ""} hover:bg-gray-400 cursor-pointer`}
            >
              Invite as guest
            </ContextMenuItem>

            <ContextMenuItem
              className={`${isAdmin ? "hidden" : ""} hover:bg-gray-400 cursor-pointer text-orange-500 hover:text-white`}
              onClick={handleAccountBlockandUnblock}
            >
              {isBlocked ? <p>Unblock account</p> : <p>Block account</p>}
            </ContextMenuItem>
            <ContextMenuItem>
              <DialogTrigger
                className={`${isAdmin ? "hidden" : ""} w-full cursor-pointer text-red-500 text-left`}
              >
                Delete
              </DialogTrigger>
            </ContextMenuItem>
          ...
          <DialogContent className="bg-gray-900 border-none">
            <DialogHeader>
              ...
              <label
                htmlFor={`Please provide reason for why you're deleting account of ${name}`}
              ...
              <input
                type="text"
                className="text-gray-400 w-full sm:w-[80%] px-[10px] py-[5px]"
                id="delete-input"
