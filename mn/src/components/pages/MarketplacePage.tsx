...
export function MarketplacePage() {
  ...
  const products = [
    ...
    },
    {
      ...
      price: 1299,
      originalPrice: 1599,
      image: 'https://images.unsplash.com/photo-1722410180670-b6d5a2e704fa?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxzYXBwaGlyZSUyMG5lY2tsYWNlJTIwbHV4dXJ5JTIwamV3ZWxyeXxlbnwxfHx8fDE3NTkwNTAwOTR8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
      rating: 4.9,
      reviews: 92,
      category: 'Necklaces',
      badge: 'Sale',
    ...
    <Card className={`group cursor-pointer overflow-hidden border-manzarri-black/10 hover:shadow-xl transition-all duration-300 ${
      ...
      <div className={`relative ${isListView ? 'w-48 flex-shrink-0' : ''}`}>
        <ImageWithFallback
          src={product.image}
          alt={product.name}
          className={`${isListView ? 'w-full h-48' : 'w-full h-64'} object-cover group-hover:scale-105 transition-transform duration-300`}
        />
        {product.badge && (
          <Badge 
            className={`absolute top-4 left-4 ${
              product.badge === 'Bestseller' ? 'bg-manzarri-reddish-brown' :
              product.badge === 'New' ? 'bg-manzarri-green' :
              product.badge === 'Limited' ? 'bg-manzarri-faun' :
              'bg-manzarri-black'
            } text-manzarri-white`}
          >
            {product.badge}
          ...
          <div className={`${isListView ? 'flex-1 pr-6' : ''}`}>
            ...
            {isListView && (
              <p className="text-manzarri-black/70 mb-4">
                Exquisite craftsmanship meets timeless design in this stunning piece that elevates any collection.
              </p>
            )}
          </div>
          <div className={`${isListView ? 'text-right' : 'flex items-center justify-between'}`}>
            <div className={`${isListView ? 'mb-4' : 'flex items-center space-x-2'}`}>
              <span className="text-2xl font-bold text-manzarri-reddish-brown">${product.price.toLocaleString()}</span>
              {product.originalPrice > product.price && (
                <span className="text-lg text-manzarri-black/50 line-through">${product.originalPrice.toLocaleString()}</span>
              )}
            </div>
            <Button 
              size="sm" 
    ...
    <div className="min-h-screen bg-manzarri-white">
      ...
      <div className="bg-manzarri-skin/20 py-12">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl font-bold text-manzarri-black mb-4">Marketplace</h1>
          <p className="text-xl text-manzarri-black/70">Discover our complete collection of luxury jewelry</p>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className={`lg:w-80 ${showFilters ? 'block' : 'hidden lg:block'}`}>
            <Card className="p-6 border-manzarri-black/10">
              <h3 className="text-xl font-semibold text-manzarri-black mb-6">Filters</h3>
              
              {/* Categories */}
              <div className="mb-8">
                <h4 className="font-medium text-manzarri-black mb-4">Categories</h4>
                <div className="space-y-3">
                  {categories.map((category) => (
                    <div key={category} className="flex items-center space-x-2">
                      <Checkbox id={category} />
                      <label htmlFor={category} className="text-sm text-manzarri-black/80">
                        {category}
                      </label>
                    </div>
                  ))}
              ...
              {/* Price Range */}
              <div className="mb-8">
                <h4 className="font-medium text-manzarri-black mb-4">Price Range</h4>
                <div className="px-2">
                  <Slider
                    value={priceRange}
              ...
              {/* Availability */}
              <div className="mb-8">
                <h4 className="font-medium text-manzarri-black mb-4">Availability</h4>
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    <Checkbox id="in-stock" />
                    <label htmlFor="in-stock" className="text-sm text-manzarri-black/80">
                      In Stock
                    </label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Checkbox id="out-of-stock" />
                    <label htmlFor="out-of-stock" className="text-sm text-manzarri-black/80">
                      Out of Stock
                    </label>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {[5, 4, 3, 2, 1].map((rating) => (
                    <div key={rating} className="flex items-center space-x-2">
                      ...
                      <label htmlFor={`rating-${rating}`} className="flex items-center text-sm text-manzarri-black/80">
                        <div className="flex items-center mr-2">
                          {[...Array(rating)].map((_, i) => (
                            <Star key={i} className="w-3 h-3 text-manzarri-faun fill-current" />
                          ))}
                        </div>
                        & Up
                      </label>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
              <div>
                <p className="text-manzarri-black/70">Showing {products.length} of {products.length} products</p>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex border border-manzarri-black/20 rounded-md">
                  <Button
                    ...
                    className={viewMode === 'grid' ? 'bg-manzarri-black text-manzarri-white' : 'text-manzarri-black'}
                  >
                  ...
                  <Button
                    ...
                    className={viewMode === 'list' ? 'bg-manzarri-black text-manzarri-white' : 'text-manzarri-black'}
                  >
              ...
              <div className="flex items-center space-x-2">
                ...
                {[1, 2, 3, 4, 5].map((page) => (
                  <Button
                    ...
                    size="sm"
                    className={page === 1 ? 'bg-manzarri-reddish-brown text-manzarri-white' : 'border-manzarri-black/20'}
                  >
                    {page}
                  </Button>
                ))}
